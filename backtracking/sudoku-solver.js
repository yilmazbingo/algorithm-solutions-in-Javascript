...
// r and c starting points, 0,0
const solveBacktrack = function (board, boxes, rows, cols, r, c) {
  if (r === board.length || c === board[0].length) {
    return true;
  } else {
    if (board[r][c] === ".") {
      // if it is blank, we put 1-9 and validate it
      for (let num = 1; num <= 9; num++) {
        // incase they ask for string
        ...
        if (isValid(box, row, col, numVal)) {
          ...
          // when we hit the end of the column, start from next row=row+1
          if (c === board[0].length - 1) {
            if (solveBacktrack(board, boxes, rows, cols, r + 1, 0)) {
              return true;
            }
          } else {
            // uptadate col=col+1 ------> from left to right
      ...
      // if the value already exist, move to the next iteration
      if (c === board[0].length - 1) {
        if (solveBacktrack(board, boxes, rows, cols, r + 1, 0)) {
          return true;
        }
...
// O(N) for brute force is 9^81. 81 cells each can take 1-9
//  with backtracking, if we have filled data we eliminate those.
// with backtracking O(N) is (9!)^9
// space complexity, what is the maximum size of the callstack. O(81)=O(1)
